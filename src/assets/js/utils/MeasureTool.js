class MeasureTool {
  ...
  setUp (params) {
    ...
    if (this.options['measureType'] === this.measureTypes.measureLength) {
      this.measureLengthClick = this.map.on('click', event => {
        let ev = this.map.on('singleclick', event => {
          if (!this.clickCount) {
            this.clickCount = this.getuuid();
      ...
      if (this.map) {
        ...
        if (this.map && vectorLayer) {
          if (params && params.hasOwnProperty('selectable')) {
            vectorLayer.set("selectable", (params.selectable ? false : true));
          }
  ...
  formatData (geom) {
    ...
    if (geom) {
      if (this.options['measureType'] === this.measureTypes.measureLength) {
        let coordinates = geom.getCoordinates(), length = 0;
